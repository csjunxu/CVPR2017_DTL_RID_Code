...
for i = 1 : par.cls_num
    ...
    Dini{i} = D;
    Dict_BID_Initial = sprintf('Data/Dict_DTL_Initial_%s.mat', datestr(now, 30));
    save(Dict_BID_Initial,'Dini');
    D = Dini{i};
    clear Dini;
    Wn = eye(size(D, 2));
    Wc = eye(size(D, 2));
    Alphan = mexLasso(XN_t, D, param);
    Alphac = mexLasso(XC_t, D, param);
    fprintf('Dictionary and Transformation Learning: Cluster: %d\n', i);
    [Alphac, Alphan, XC_t, XN_t, D, Wc, Wn, Uc, Un, Vn, f] = DTL_ADPU_20160928(Alphac, Alphan, XC_t, XN_t, D, Wc, Wn, par);
    Dict.D{i} = D;
    Dict.WC{i} = Wc;
    Dict.WN{i} = Wn;
    Dict.UC{i} = Uc;
