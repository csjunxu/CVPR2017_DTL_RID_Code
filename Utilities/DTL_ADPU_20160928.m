...
function [Alphap, Alphas, Xp, Xs, D, Wp, Ws, Up, Us, Vs, f] = DTL_ADPU_20160928(Alphap, Alphas, Xp, Xs, D, Wp, Ws, par)
...
for t = 1 : par.nIter
    ...
    f_prev = f;
    Alphas = mexLasso([Xs - Vs; par.sqrtmu * Up * full(Alphap)], [D; par.sqrtmu * Us],param);
    Alphap = mexLasso([Xp; par.sqrtmu * Us * full(Alphas)], [D; par.sqrtmu * Up],param);
    dictSize = par.K;
    
    %% Updating D
    for i=1:dictSize
        ai        =    [Alphas(i,:) Alphap(i,:)];
        Y         =   [Xs - Vs Xp] - D * [Alphas Alphap] + D(:,i) * ai;
        di        =    Y * ai';
        di        =    di ./ (norm(di,2) + eps);
        D(:,i)   =    di;
    end
