function [im_out, PN] = DTL_ADPU_BID_full(IMin_y,model,Dict,par,param)
[h,w,ch] = size(IMin_y);
...
for t = 1 : par.nInnerLoop
    ...
    for i = 1 : par.cls_num
        ...
        if (t == 1)
            Pn    = zeros(size(Xn));
            Alphan = mexLasso(Xn - Pn, D, param);
            Alphac = Uc \ Un * Alphan;
            Xc = D * Alphac; % Xc->Xn; 07/07/2016;  Xn->Xc; 07/22/2016;
        else
            Pn = PN{i};
            Alphac = AC(:, idx_cluster);
        end
        D2 = [D; par.sqrtmu * Un]; % Wn ->Un 07/22/2016;
        ...
        Alphan = mexLasso(Y, D2,param);
        Pn = Xn - D * Alphan; % 08/17/2016;
        PN{i} = Pn;
        clear Y D2;
        %% CVPR2012 SCDL case
        D3 = [D; par.sqrtmu * Uc];
