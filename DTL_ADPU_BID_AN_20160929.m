...
for i = 1 : im_num
    ...
    while Continue
        fprintf('Iter: %d \n', nOuterLoop);
        IMout_y = DTL_ADPU_BID_full(IMin_y,model,Dict,par,param);
    ...
    %% output
    imwrite(IMout, ['../cc_Results/Real_DTL/DTL_BID_AN_' IMname '.png']);
end
mPSNR = mean(PSNR);
mSSIM = mean(SSIM);
mCCPSNR = mean(CCPSNR);
mCCSSIM = mean(CCSSIM);
save(['C:\Users\csjunxu\Desktop\CVPR2017\cc_Results\Real_DTL.mat'],'PSNR','mPSNR','SSIM','mSSIM','CCPSNR','mCCPSNR','CCSSIM','mCCSSIM');
